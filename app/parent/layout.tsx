...
export default function ParentLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    async function checkAuth() {
      try {
        ...
        setParentName(data.parent.fatherName);
        setStudentName(data.student?.studentName || null);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#1B3A5C] text-white transform transition-transform duration-200 ease-in-out ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 lg:static lg:inset-auto`}>
        <div className="h-full flex flex-col">
          <div className="px-6 py-5 border-b border-white/10">
            <div className="flex items-center gap-3">
              <NextImage src="/school-logo.png" alt="Adarsh High School" width={32} height={32} className="w-8 h-8 rounded-full object-contain" />
              <div>
                <h2 className="font-semibold text-sm">Parent Portal</h2>
                <p className="text-xs text-white/60">{studentName || "Student"}</p>
              </div>
            </div>
          </div>
            ...
            {sidebarLinks.map((link) => {
              ...
              return (
                <Link key={link.href} href={link.href} onClick={() => setSidebarOpen(false)}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors ${isActive ? "bg-white/15 text-white font-medium" : "text-white/70 hover:bg-white/10 hover:text-white"}`}>
                  <Icon className="w-5 h-5 flex-shrink-0" /> {link.label}
                </Link>
              );
            })}
          </nav>
          <div className="p-3 border-t border-white/10">
            <button onClick={handleLogout} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-white/70 hover:bg-white/10 hover:text-white w-full transition-colors">
