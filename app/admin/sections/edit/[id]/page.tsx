  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/sections/${id}`, {
        ...
        body: JSON.stringify({ sectionName: sectionName.trim(), status }),
      });
  ...
  if (notFound) {
    return (
      <div className="text-center py-12">
        <h3 className="text-lg font-medium text-gray-900 mb-1">Section not found</h3>
        <Link href="/admin/sections" className="text-[#FF9933] text-sm hover:underline">Back to Sections</Link>
      </div>
    );
  }
